...
1. `packages/db` **tenant-scoped data-access layer** over the schema above: every query org-scoped, no cross-tenant path (`[arch §5.2]`, `[invariant 3]`); + wire BetterAuth route handler in `apps/backend`. (Schema + migrations done.)
2. Secrets / `KeyProvider`: env-sourced KEK + AES-256-GCM envelope encrypt/decrypt; the **encrypted-secret tables** (`model_key` BYOK, `app_credential`, per-org wrapped-DEK) land here, where the envelope shape is decided - deliberately deferred from the schema pass (`[tech-arch §6]`).
